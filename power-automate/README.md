...
- `AtlasBridge_1_0_0_0.zip`: plantilla base de la solución no administrada AtlasBridge 1.14 para el asistente actual. Instala dos flujos activos con las mismas referencias de conexión de Outlook, Teams y OneDrive. El asistente la personaliza en el PC con el identificador persistente de la instalación antes de entregarla: la solución importada se llama `AtlasBridge_<id de instalación>` y usa referencias `atlas_*_<id de instalación>` y GUID de flujo derivados, de modo que cada usuario del mismo entorno instala una solución independiente y una nueva versión de Atlas actualiza la solución de esa instalación en lugar de crear otra. La fecha histórica es opcional: si `selected-date.txt` no existe o está vacío, el flujo continúa con el día actual. El contenido de ese archivo se lee como texto plano (así lo devuelve OneDrive para un .txt) con respaldo para un cuerpo `$content` en base64. El nombre estable permite que el cliente la encuentre; la versión interna controla la actualización.
...
- **Atlas - Export evidence to OneDrive** se ejecuta cada cinco minutos para correo y calendario. Lee `AtlasBridge/requests/selected-date.txt`: si Atlas solicitó una fecha usa esa fecha; si está vacío usa el día actual de Bogotá. También conserva un respaldo de Teams con hasta 25 chats activos del día: List chats sigue la paginación del conector para no perder conversaciones en una primera página arbitraria, la política de reintentos exponenciales absorbe la limitación de Teams sin pausas fijas y solicita hasta 40 mensajes del día por chat. La captura por evento sigue guardando mensajes de los demás chats sin recorrer una lista de 100 conversaciones.
...
El flujo programado consulta hasta 500 eventos, los 100 correos más recientes y hasta 40 mensajes del día por chat en un máximo de 25 chats activos. **List chats** declara la política de paginación del conector (hasta 500 conversaciones) para que el filtro por día se aplique sobre la lista completa y no sobre una página arbitraria. La cobertura principal de Teams usa el disparador por mensaje y consulta el chat identificado por el evento, aunque el usuario participe en más de 100 conversaciones durante el día.
