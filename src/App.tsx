...
function DestinationSetupScreen({ initial, initialAutoSync, bridgeMode, onSave, onCancel }: { initial?: TrackerDestination; initialAutoSync: boolean; bridgeMode: boolean; onSave: (destination: TrackerDestination, autoSync: boolean) => Promise<void>; onCancel?: () => void }) {
  ...
  const options: { kind: TrackerDestinationKind; title: string; detail: string; icon: React.ReactNode }[] = [
    ...
    ...(!bridgeMode ? [{ kind: "share_point" as const, title: "SharePoint link", detail: "Update the online .xlsx or .xlsm directly", icon: <Cloud className="h-5 w-5" /> }] : []),
  ];
  return <main className="mx-auto min-h-screen max-w-6xl px-10 py-10">
    ...
    <form onSubmit={submit} className="card mx-auto mt-10 max-w-4xl p-9">
      ...
      <label className="mt-7 flex items-start gap-3 rounded-2xl border border-pine/15 bg-mint/35 p-4"><input type="checkbox" checked={autoSync} onChange={event => setAutoSync(event.target.checked)} className="mt-0.5 h-4 w-4 accent-pine" /><span><b className="block text-sm">Automatic {bridgeMode ? "inbox import" : "calendar sync"}</b><span className="mt-1 block text-xs leading-5 text-ink/50">{bridgeMode ? "Import the newest package for today when Atlas opens, then every hour while it remains running." : "Sync today when Atlas opens, then every hour while it remains running. Manual tasks stay optional."}</span></span></label>
      {error && <div className="mt-5"><ErrorBanner message={error} /></div>}
...
function SourceBadge({ item }: { item: Interaction }) {
  ...
  return <span className={`chip ${styles}`}><Icon className="h-3 w-3" />{item.sourceKind === "teams_chat" ? (item.aiSuggested ? "AI · unverified" : "Teams evidence") : item.sourceKind.replace("_", " ")}</span>;
}
...
function ExportPanel({ destination, autoSync, bridgeMode, items, busy, onSave, onSync, onEdit }: { destination: TrackerDestination; autoSync: boolean; bridgeMode: boolean; items: Interaction[]; busy: boolean; onSave: () => void; onSync: () => void; onEdit: () => void }) {
    ...
    <main className="mx-auto max-w-[1540px] px-8 py-8">
      <div className="grid grid-cols-[minmax(0,1fr)_290px] gap-6">
        <section className="min-w-0">
          ...
          <div className="mt-6 flex items-center justify-between"><div><h2 className="font-display text-2xl">Preview</h2><p className="mt-1 text-xs text-ink/45">Mail and Teams evidence remain unchecked until you confirm the real interaction.</p></div><button className="btn-secondary" onClick={() => setManual(true)}><Plus className="h-4 w-4" />Añadir interacción manual</button></div>
