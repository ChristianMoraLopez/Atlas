...
export default function ConnectorInstaller({ onClose, onUseFolder }: { onClose?: () => void; onUseFolder: (folder: string) => Promise<void> }) {
  ...
  }, t(actionLabels[String(action.kind)] ?? 'Procesando la configuración'));
  useEffect(() => {
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-10 py-8" aria-busy={busy || !snapshot}>
    ...
    <div className="mt-7 grid grid-cols-[280px_1fr] gap-6">
      <ol className="card space-y-3 p-5" aria-label={t('Etapas de instalación')}>{steps.map(([key, label], i) => <li key={key} aria-current={key === phase ? 'step' : undefined} className={`flex gap-3 text-sm ${key === phase ? 'font-bold text-pine' : 'text-ink/50'}`}><span>{i + 1}.</span>{t(label)}{key === 'completed' && phase === key && <Check className="h-4 w-4" />}</li>)}</ol>
      <section className="card min-w-0 p-7">
        ...
        {phase === 'checking_requirements' && <div className="mt-5 space-y-4">
          <p className="text-sm">{t('Necesitas un entorno con Dataverse, Environment Maker o permisos equivalentes para crear el flujo, privilegios de importación y conexiones propias de Outlook, Teams y OneDrive. Estos requisitos se comprueban en Microsoft; Atlas no los da por aprobados.')}</p>
          ...
          <button className="btn-primary" disabled={busy || !root || !syncAccount || !snapshot} onClick={() => void act({ kind: 'prepare', one_drive_root: root, calendar_name: '' })}>{t('Preparar carpeta y solución')}</button>
        </div>}
        ...
        {phase === 'finding_connections' && <div className="mt-5 space-y-4"><p className="text-sm">{t('En Microsoft, revisa tus conexiones de')} <b>Office 365 Outlook, Microsoft Teams y OneDrive for Business</b>. {t('Durante la importación selecciona las que pertenezcan a tu cuenta. Si Microsoft solicita crear o reparar una conexión, completa esa pantalla oficial y vuelve aquí.')}</p><label className="flex items-start gap-2 text-sm"><input type="checkbox" checked={ownConnections} onChange={e => setOwnConnections(e.target.checked)} />{t('Confirmo que las tres conexiones son mías, están autorizadas y OneDrive coincide con la carpeta elegida.')}</label><button className="btn-primary" disabled={busy || !ownConnections} onClick={() => void act({ kind: 'confirm_connections' })}>{t('Continuar a importación')}</button></div>}
        {phase === 'importing_solution' && <div className="mt-5 space-y-4"><p className="text-sm">{t('En Soluciones → Importar solución, carga el ZIP preparado por Atlas, asocia Outlook, Teams y OneDrive con tu cuenta Circana y pulsa Importar. El paquete instala tu propia solución AtlasBridge identificada por esta instalación de Atlas: si ya la importaste antes se actualiza, y nunca modifica la de otros usuarios del entorno.')}</p><p className="break-all rounded-xl bg-cream p-3 text-xs">{snapshot?.packagePath}</p><button className="btn-secondary" disabled={busy} onClick={() => void run(async () => { await invoke('connector_installer_show_package'); })}><FolderOpen className="h-4 w-4" />{t('Mostrar ZIP')}</button><button className="btn-primary ml-3" disabled={busy} onClick={() => void act({ kind: 'confirm_import' })}>{t('Microsoft confirmó la importación')}</button></div>}
